...
pub const FlatVal = opaque {
    ...
    pub const Initializer = struct {
        ...
        /// int function(Ihandle *ih, int start); [in C]elem:valuechanging_cb(start:
        /// number) -> (ret: number) [in Lua]
        pub fn setValueChangIngCallback(self: Initializer, callback: ?*const OnValueChangIngFn) Initializer {
        ...
        pub fn setFlatWheelCallback(self: Initializer, callback: ?*const OnFlatWheelFn) Initializer {
    ...
    /// int function(Ihandle *ih, int start); [in C]elem:valuechanging_cb(start:
    /// number) -> (ret: number) [in Lua]
    pub fn setValueChangIngCallback(self: *Self, callback: ?*const OnValueChangIngFn) void {
        const Handler = CallbackHandler(Self, OnValueChangIngFn, "VALUECHANGING_CB");
        Handler.setCallback(self, callback);
    }
    ...
    pub fn setFlatWheelCallback(self: *Self, callback: ?*const OnFlatWheelFn) void {
        const Handler = CallbackHandler(Self, OnFlatWheelFn, "FLAT_WHEEL_CB");
        Handler.setCallback(self, callback);
    }
...
test "FlatVal TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatVal.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "FlatVal TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatVal.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
