...
test "FlatLabel TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatLabel.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "FlatLabel TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatLabel.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
