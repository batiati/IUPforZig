...
pub const FlatFrame = opaque {
    pub const CLASS_NAME = "flatframe";
    const Self = @This();
    ...
    /// Default: Yes.
    /// If value is CROSSTITLE the the frame at top crosses the title, like
    /// traditional frames in native systems (since 3.25).
    /// When CROSSTITLE is used TITLELINE and TITLEALIGNMENT are ignored, the title
    /// line is never drawn and alignment is always left.
    ...
    pub const Initializer = struct {
        ...
        /// When CROSSTITLE is used TITLELINE and TITLEALIGNMENT are ignored, the title
        /// line is never drawn and alignment is always left.
        pub fn setFrame(self: *Initializer, arg: ?Frame) Initializer {
            if (arg) |value| switch (value) {
                .Yes => c.setStrAttribute(self.ref, "FRAME", void, void, "YES"),
                .No => c.setStrAttribute(self.ref, "FRAME", void, void, "NO"),
                .CrossTitle => c.setStrAttribute(self.ref, "FRAME", void, void, "CROSSTITLE"),
            } else {
                c.clearAttribute(self.ref, "FRAME", void, void);
            }
            return self.*;
        }
    ...
    pub fn getFrame(self: *Self) ?Frame {
        var ret = c.getStrAttribute(self, "FRAME", void, void);
    ...
    /// When CROSSTITLE is used TITLELINE and TITLEALIGNMENT are ignored, the title
    /// line is never drawn and alignment is always left.
    pub fn setFrame(self: *Self, arg: ?Frame) void {
        if (arg) |value| switch (value) {
            .Yes => c.setStrAttribute(self, "FRAME", void, void, "YES"),
            .No => c.setStrAttribute(self, "FRAME", void, void, "NO"),
            .CrossTitle => c.setStrAttribute(self, "FRAME", void, void, "CROSSTITLE"),
        } else {
            c.clearAttribute(self, "FRAME", void, void);
        }
    }
...
test "FlatFrame TipBalloon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipBalloon(true).unwrap());
    defer item.deinit();
...
test "FlatFrame HandleName" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setHandleName("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame CanvasBox" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setCanvasBox(true).unwrap());
    defer item.deinit();
...
test "FlatFrame TipBgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipBgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "FlatFrame XMin" {
    ...
    var item = try (iup.FlatFrame.init().setXMin(42).unwrap());
    defer item.deinit();
...
test "FlatFrame BackColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setBackColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "FlatFrame MaxSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setMaxSize(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawTextWrap" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawTextWrap(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawUsedIRect2d" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawUsedIRect2d(true).unwrap());
    defer item.deinit();
...
test "FlatFrame TitleImage" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTitleImage("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame Position" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setPosition(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame DropFilesTarget" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDropFilesTarget(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawTextAlignment" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawTextAlignment(.ACenter).unwrap());
    defer item.deinit();
...
test "FlatFrame Tip" {
    ...
    var item = try (iup.FlatFrame.init().setTip("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame DrawTextLayoutCenter" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawTextLayoutCenter(true).unwrap());
    defer item.deinit();
...
test "FlatFrame CanFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setCanFocus(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DragSourceMove" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDragSourceMove(true).unwrap());
    defer item.deinit();
...
test "FlatFrame Visible" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setVisible(true).unwrap());
    defer item.deinit();
...
test "FlatFrame LineX" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setLineX(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame Cursor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setCursor("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame LineY" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setLineY(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawLineWidth" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawLineWidth(42).unwrap());
    defer item.deinit();
...
test "FlatFrame DragDrop" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDragDrop(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawAntialias" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawAntialias(42).unwrap());
    defer item.deinit();
...
test "FlatFrame Theme" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTheme("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame DragCursorCopy" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDragCursorCopy("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame HtTransparent" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setHtTransparent(true).unwrap());
    defer item.deinit();
...
test "FlatFrame ChildOffset" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setChildOffset(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame Expand" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setExpand(.Yes).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawFont" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawFont("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame Size" {
    ...
    var item = try (iup.FlatFrame.init().setSize(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame PosX" {
    ...
    var item = try (iup.FlatFrame.init().setPosX(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame PosY" {
    ...
    var item = try (iup.FlatFrame.init().setPosY(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame YMin" {
    ...
    var item = try (iup.FlatFrame.init().setYMin(42).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawMakeInactive" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawMakeInactive(true).unwrap());
    defer item.deinit();
...
test "FlatFrame FontSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setFontSize(42).unwrap());
    defer item.deinit();
...
test "FlatFrame DropTypes" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDropTypes("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame UserSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setUserSize(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame TipDelay" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipDelay(42).unwrap());
    defer item.deinit();
...
test "FlatFrame ScrollBar" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setScrollBar(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DragStart" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDragStart(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame Title" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTitle("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame XAutoHide" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setXAutoHide(true).unwrap());
    defer item.deinit();
...
test "FlatFrame PropagateFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setPropagateFocus(true).unwrap());
    defer item.deinit();
...
test "FlatFrame XMax" {
    ...
    var item = try (iup.FlatFrame.init().setXMax(42).unwrap());
    defer item.deinit();
...
test "FlatFrame BgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setBgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "FlatFrame TipBalloonTitle" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipBalloonTitle("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame DropTarget" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDropTarget(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DX" {
    ...
    var item = try (iup.FlatFrame.init().setDX(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame DY" {
    ...
    var item = try (iup.FlatFrame.init().setDY(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawTextEllipsis" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawTextEllipsis(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DragSource" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDragSource(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawTextClip" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawTextClip(true).unwrap());
    defer item.deinit();
...
test "FlatFrame Floating" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setFloating(.Yes).unwrap());
    defer item.deinit();
...
test "FlatFrame NormalizerGroup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setNormalizerGroup("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame RasterSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setRasterSize(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame TipFgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipFgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "FlatFrame ControlId" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setControlId(42).unwrap());
    defer item.deinit();
...
test "FlatFrame FrameColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setFrameColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "FlatFrame FontFace" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setFontFace("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame DrawColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawTextOrientation" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawTextOrientation(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawBgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawBgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "FlatFrame Name" {
    ...
    var item = try (iup.FlatFrame.init().setName("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame BackingStore" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setBackingStore(true).unwrap());
    defer item.deinit();
...
test "FlatFrame TipBalloonTitleIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipBalloonTitleIcon(true).unwrap());
    defer item.deinit();
...
test "FlatFrame YAutoHide" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setYAutoHide(true).unwrap());
    defer item.deinit();
...
test "FlatFrame DrawStyle" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDrawStyle(.Fill).unwrap());
    defer item.deinit();
...
test "FlatFrame BackImageZoom" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setBackImageZoom(true).unwrap());
    defer item.deinit();
...
test "FlatFrame Active" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setActive(true).unwrap());
    defer item.deinit();
...
test "FlatFrame TipVisible" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipVisible(true).unwrap());
    defer item.deinit();
...
test "FlatFrame YMax" {
    ...
    var item = try (iup.FlatFrame.init().setYMax(42).unwrap());
    defer item.deinit();
...
test "FlatFrame BackImage" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setBackImage("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame ExpandWeight" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setExpandWeight(3.14).unwrap());
    defer item.deinit();
...
test "FlatFrame MinSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setMinSize(9, 10).unwrap());
    defer item.deinit();
...
test "FlatFrame NTheme" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setNTheme("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame DragTypes" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDragTypes("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame WheelDropFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setWheelDropFocus(true).unwrap());
    defer item.deinit();
...
test "FlatFrame FontStyle" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setFontStyle("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame Touch" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTouch(true).unwrap());
    defer item.deinit();
...
test "FlatFrame FrameWidth" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setFrameWidth(42).unwrap());
    defer item.deinit();
...
test "FlatFrame Frame" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setFrame(.Yes).unwrap());
    defer item.deinit();
...
test "FlatFrame DragCursor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setDragCursor("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame Font" {
    ...
    var item = try (iup.FlatFrame.init().setFont("Hello").unwrap());
    defer item.deinit();
