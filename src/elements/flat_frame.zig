...
test "FlatFrame TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "FlatFrame TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatFrame.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
