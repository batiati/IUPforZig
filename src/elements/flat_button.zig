...
test "FlatButton TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatButton.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "FlatButton TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatButton.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
