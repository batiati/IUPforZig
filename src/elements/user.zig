...
/// Its use is usually for additional elements, but you can use it to create an
/// Ihandle* to store private attributes.
/// It is also a void container.
/// Its children can be dynamically added using IupAppend or IupInsert.
pub const User = opaque {
    pub const CLASS_NAME = "user";
    const Self = @This();
