...
pub const FlatTabs = opaque {
    ...
    pub const Initializer = struct {
        ...
        pub fn setTabOrientation(self: *Initializer, arg: ?TabOrientation) Initializer {
            if (arg) |value| switch (value) {
                ...
                .Vertical => c.setStrAttribute(self.ref, "TABORIENTATION", "VERTICAL"),
            } else {
                c.clearAttribute(self.ref, "TABORIENTATION");
            }
            return self.*;
        }
    ...
    pub fn setTabOrientation(self: *Self, arg: ?TabOrientation) void {
        if (arg) |value| switch (value) {
            ...
            .Vertical => c.setStrAttribute(self, "TABORIENTATION", "VERTICAL"),
        } else {
            c.clearAttribute(self, "TABORIENTATION");
        }
    }
...
test "FlatTabs TabTitle" {
    ...
    var item = try (iup.FlatTabs.init().setTabTitle("Hello").unwrap());
    defer item.deinit();
...
test "FlatTabs TabFontSize" {
    ...
    var item = try (iup.FlatTabs.init().setTabFontSize(42).unwrap());
    defer item.deinit();

    var ret = item.getTabFontSize();
...
test "FlatTabs TabVisible" {
    ...
    var item = try (iup.FlatTabs.init().setTabVisible(true).unwrap());
    defer item.deinit();
...
test "FlatTabs TabImage" {
    ...
    var item = try (iup.FlatTabs.init().setTabImage("Hello").unwrap());
    defer item.deinit();
