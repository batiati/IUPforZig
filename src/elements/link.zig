...
pub const Link = opaque {
    ...
    pub const Initializer = struct {
        ...
        pub fn setUrl(self: *Initializer, arg: [:0]const u8) Initializer {
            c.setStrAttribute(self.ref, "URL", void, void, arg);
            return self.*;
        }
    ...
    pub fn getUrl(self: *Self) [:0]const u8 {
        return c.getStrAttribute(self, "URL", void, void);
    }
    ...
    pub fn setUrl(self: *Self, arg: [:0]const u8) void {
        c.setStrAttribute(self, "URL", void, void, arg);
    }
...
test "Link TipBalloon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Link.init().setTipBalloon(true).unwrap());
    defer item.deinit();
...
test "Link Image" {
    ...
    var item = try (iup.Link.init().setImage("Hello").unwrap());
    defer item.deinit();
...
test "Link Cursor" {
    ...
    var item = try (iup.Link.init().setCursor("Hello").unwrap());
    defer item.deinit();
...
test "Link DragCursorCopy" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Link.init().setDragCursorCopy("Hello").unwrap());
    defer item.deinit();
...
test "Link HtTransparent" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Link.init().setHtTransparent(true).unwrap());
    defer item.deinit();
...
test "Link DragStart" {
    ...
    var item = try (iup.Link.init().setDragStart(9, 10).unwrap());
    defer item.deinit();
...
test "Link TipBalloonTitle" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Link.init().setTipBalloonTitle("Hello").unwrap());
    defer item.deinit();
...
test "Link ControlId" {
    ...
    var item = try (iup.Link.init().setControlId(42).unwrap());
    defer item.deinit();
...
test "Link TipBalloonTitleIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Link.init().setTipBalloonTitleIcon(true).unwrap());
    defer item.deinit();
...
test "Link Touch" {
    ...
    var item = try (iup.Link.init().setTouch(true).unwrap());
    defer item.deinit();
...
test "Link DragCursor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Link.init().setDragCursor("Hello").unwrap());
    defer item.deinit();
