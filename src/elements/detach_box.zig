...
/// Dragging and dropping this element, it creates a new dialog composed by its
/// child or elements arranged in it (for example, a child like IupVbox or IupHbox).
...
pub const DetachBox = opaque {
    pub const CLASS_NAME = "detachbox";
    ...
    pub const Initializer = struct {
        ...
        /// Default: "160 160 160".
        /// When SHOWGRIP = NO, this attribute sets the background color of the bar handler.
        ...
        /// int function(Ihandle *ih, Ihandle *new_parent, int x, int y); [in C]
        /// ih:detached_cb(new_parent: ihandle, x, y: number) -> (ret: number) [in Lua]
        pub fn setDetachedCallback(self: Initializer, callback: ?*const OnDetachedFn) Initializer {
        ...
        /// [in C] ih:restored_cb(old_parent: ihandle, x, y: number) -> (ret: number)
        /// [in Lua]
        pub fn setRestoredCallback(self: Initializer, callback: ?*const OnRestoredFn) Initializer {
    ...
    pub fn getOldBrotherHandle(self: *Self) ?iup.Element {
        if (interop.getHandleAttribute(self, "OLDBROTHER_HANDLE", .{})) |handle| {
    ...
    /// Only valid after the element was detached.
    pub fn getOldParentHandle(self: *Self) ?iup.Element {
        if (interop.getHandleAttribute(self, "OLDPARENT_HANDLE", .{})) |handle| {
            return iup.Element.fromHandle(handle);
        } else {
            return null;
        }
    ...
    /// int function(Ihandle *ih, Ihandle *new_parent, int x, int y); [in C]
    /// ih:detached_cb(new_parent: ihandle, x, y: number) -> (ret: number) [in Lua]
    pub fn setDetachedCallback(self: *Self, callback: ?*const OnDetachedFn) void {
        const Handler = CallbackHandler(Self, OnDetachedFn, "DETACHED_CB");
        Handler.setCallback(self, callback);
    }
    ...
    /// [in C] ih:restored_cb(old_parent: ihandle, x, y: number) -> (ret: number)
    /// [in Lua]
    pub fn setRestoredCallback(self: *Self, callback: ?*const OnRestoredFn) void {
        const Handler = CallbackHandler(Self, OnRestoredFn, "RESTORED_CB");
        Handler.setCallback(self, callback);
    }
};

test "DetachBox UserSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setUserSize(9, 10).unwrap());
    defer item.deinit();
...
test "DetachBox FontStyle" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setFontStyle("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox FontSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setFontSize(42).unwrap());
    defer item.deinit();
...
test "DetachBox BarSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setBarSize(42).unwrap());
    defer item.deinit();
...
test "DetachBox ExpandWeight" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setExpandWeight(3.14).unwrap());
    defer item.deinit();
...
test "DetachBox MaxSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setMaxSize(9, 10).unwrap());
    defer item.deinit();
...
test "DetachBox Active" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setActive(true).unwrap());
    defer item.deinit();
...
test "DetachBox NTheme" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setNTheme("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox Visible" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setVisible(true).unwrap());
    defer item.deinit();
...
test "DetachBox FontFace" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setFontFace("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox PropagateFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setPropagateFocus(true).unwrap());
    defer item.deinit();
...
test "DetachBox HandleName" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setHandleName("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox Size" {
    ...
    var item = try (iup.DetachBox.init().setSize(9, 10).unwrap());
    defer item.deinit();
...
test "DetachBox NormalizerGroup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setNormalizerGroup("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox Font" {
    ...
    var item = try (iup.DetachBox.init().setFont("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox Name" {
    ...
    var item = try (iup.DetachBox.init().setName("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox Expand" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setExpand(.Yes).unwrap());
    defer item.deinit();
...
test "DetachBox CanFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setCanFocus(true).unwrap());
    defer item.deinit();
...
test "DetachBox RasterSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setRasterSize(9, 10).unwrap());
    defer item.deinit();
...
test "DetachBox Color" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "DetachBox Floating" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setFloating(.Yes).unwrap());
    defer item.deinit();
...
test "DetachBox ShowGrip" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setShowGrip(true).unwrap());
    defer item.deinit();
...
test "DetachBox Theme" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setTheme("Hello").unwrap());
    defer item.deinit();
...
test "DetachBox MinSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setMinSize(9, 10).unwrap());
    defer item.deinit();
...
test "DetachBox Position" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.DetachBox.init().setPosition(9, 10).unwrap());
    defer item.deinit();
