...
test "Calendar TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Calendar.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "Calendar TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Calendar.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
