...
/// It can select one or two colors.
/// The primary color is selected with the left mouse button, and the secondary
/// color is selected with the right mouse button.
/// You can double click a cell to change its color and you can double click
/// the preview area to switch between primary and secondary colors.
...
pub const ColorBar = opaque {
    pub const CLASS_NAME = "colorbar";
    ...
    pub const Initializer = struct {
        ...
        pub fn setShowPreview(self: Initializer, arg: bool) Initializer {
            if (self.last_error) |_| return self;
            interop.setBoolAttribute(self.ref, "SHOW_PREVIEW", .{}, arg);
            return self;
        }
        ...
        /// int function(Ihandle* ih, int cell, int type); [in C] ih:select_cb(cell,
        /// type: number) -> (ret: number) [in Lua]
        pub fn setSelectCallback(self: Initializer, callback: ?*const OnSelectFn) Initializer {
        ...
        /// int function(Ihandle* ih, int prim_cell, int sec_cell); [in C]
        /// ih:switch_cb(prim_cell, sec_cell: number) -> (ret: number) [in Lua]
        pub fn setSwitchCallback(self: Initializer, callback: ?*const OnSwitchFn) Initializer {
        ...
        /// int function(Ihandle* ih, int cell); [in C] ih:extended_cb(cell: number) ->
        /// (ret: number) [in Lua]
        pub fn setExtEndedCallback(self: Initializer, callback: ?*const OnExtEndedFn) Initializer {
        ...
        /// char* function(Ihandle* ih, int cell); [in C] ih:cell_cb(cell: number) ->
        /// (ret: string) [in Lua]
        pub fn setCellCallback(self: Initializer, callback: ?*const OnCellFn) Initializer {
    ...
    pub fn setShowPreview(self: *Self, arg: bool) void {
        interop.setBoolAttribute(self, "SHOW_PREVIEW", .{}, arg);
    }
    ...
    /// int function(Ihandle* ih, int cell, int type); [in C] ih:select_cb(cell,
    /// type: number) -> (ret: number) [in Lua]
    pub fn setSelectCallback(self: *Self, callback: ?*const OnSelectFn) void {
        const Handler = CallbackHandler(Self, OnSelectFn, "SELECT_CB");
        Handler.setCallback(self, callback);
    }
    ...
    /// int function(Ihandle* ih, int prim_cell, int sec_cell); [in C]
    /// ih:switch_cb(prim_cell, sec_cell: number) -> (ret: number) [in Lua]
    pub fn setSwitchCallback(self: *Self, callback: ?*const OnSwitchFn) void {
        const Handler = CallbackHandler(Self, OnSwitchFn, "SWITCH_CB");
        Handler.setCallback(self, callback);
    }
    ...
    /// int function(Ihandle* ih, int cell); [in C] ih:extended_cb(cell: number) ->
    /// (ret: number) [in Lua]
    pub fn setExtEndedCallback(self: *Self, callback: ?*const OnExtEndedFn) void {
        const Handler = CallbackHandler(Self, OnExtEndedFn, "EXTENDED_CB");
        Handler.setCallback(self, callback);
    }
    ...
    /// char* function(Ihandle* ih, int cell); [in C] ih:cell_cb(cell: number) ->
    /// (ret: string) [in Lua]
    pub fn setCellCallback(self: *Self, callback: ?*const OnCellFn) void {
        const Handler = CallbackHandler(Self, OnCellFn, "CELL_CB");
        Handler.setCallback(self, callback);
    }
...
test "ColorBar HandleName" {
    ...
    var item = try (iup.ColorBar.init().setHandleName("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar TipBgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setTipBgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "ColorBar XMin" {
    ...
    var item = try (iup.ColorBar.init().setXMin(42).unwrap());
    defer item.deinit();
...
test "ColorBar MaxSize" {
    ...
    var item = try (iup.ColorBar.init().setMaxSize(9, 10).unwrap());
    defer item.deinit();
...
test "ColorBar ShowPreview" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setShowPreview(true).unwrap());
    defer item.deinit();
...
test "ColorBar DrawTextWrap" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawTextWrap(true).unwrap());
    defer item.deinit();
...
test "ColorBar Position" {
    ...
    var item = try (iup.ColorBar.init().setPosition(9, 10).unwrap());
    defer item.deinit();
...
test "ColorBar DropFilesTarget" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDropFilesTarget(true).unwrap());
    defer item.deinit();
...
test "ColorBar DrawTextAlignment" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawTextAlignment(.ACenter).unwrap());
    defer item.deinit();
...
test "ColorBar Tip" {
    ...
    var item = try (iup.ColorBar.init().setTip("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar DrawTextLayoutCenter" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawTextLayoutCenter(true).unwrap());
    defer item.deinit();
...
test "ColorBar CanFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setCanFocus(true).unwrap());
    defer item.deinit();
...
test "ColorBar DragSourceMove" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDragSourceMove(true).unwrap());
    defer item.deinit();
...
test "ColorBar Visible" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setVisible(true).unwrap());
    defer item.deinit();
...
test "ColorBar LineX" {
    ...
    var item = try (iup.ColorBar.init().setLineX(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar LineY" {
    ...
    var item = try (iup.ColorBar.init().setLineY(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar DrawLineWidth" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawLineWidth(42).unwrap());
    defer item.deinit();
...
test "ColorBar DragDrop" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDragDrop(true).unwrap());
    defer item.deinit();
...
test "ColorBar Theme" {
    ...
    var item = try (iup.ColorBar.init().setTheme("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar FlatColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setFlatColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "ColorBar Expand" {
    ...
    var item = try (iup.ColorBar.init().setExpand(.Yes).unwrap());
    defer item.deinit();
...
test "ColorBar DrawFont" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawFont("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar Size" {
    ...
    var item = try (iup.ColorBar.init().setSize(9, 10).unwrap());
    defer item.deinit();
...
test "ColorBar PosX" {
    ...
    var item = try (iup.ColorBar.init().setPosX(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar PosY" {
    ...
    var item = try (iup.ColorBar.init().setPosY(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar YMin" {
    ...
    var item = try (iup.ColorBar.init().setYMin(42).unwrap());
    defer item.deinit();
...
test "ColorBar Orientation" {
    ...
    var item = try (iup.ColorBar.init().setOrientation(.Horizontal).unwrap());
    defer item.deinit();
...
test "ColorBar DrawMakeInactive" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawMakeInactive(true).unwrap());
    defer item.deinit();
...
test "ColorBar FontSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setFontSize(42).unwrap());
    defer item.deinit();
...
test "ColorBar DropTypes" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDropTypes("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar UserSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setUserSize(9, 10).unwrap());
    defer item.deinit();
...
test "ColorBar TipDelay" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setTipDelay(42).unwrap());
    defer item.deinit();
...
test "ColorBar ScrollBar" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setScrollBar(true).unwrap());
    defer item.deinit();
...
test "ColorBar XAutoHide" {
    ...
    var item = try (iup.ColorBar.init().setXAutoHide(true).unwrap());
    defer item.deinit();
...
test "ColorBar PropagateFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setPropagateFocus(true).unwrap());
    defer item.deinit();
...
test "ColorBar XMax" {
    ...
    var item = try (iup.ColorBar.init().setXMax(42).unwrap());
    defer item.deinit();
...
test "ColorBar BgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setBgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "ColorBar DropTarget" {
    ...
    var item = try (iup.ColorBar.init().setDropTarget(true).unwrap());
    defer item.deinit();
...
test "ColorBar DX" {
    ...
    var item = try (iup.ColorBar.init().setDX(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar DY" {
    ...
    var item = try (iup.ColorBar.init().setDY(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar DrawTextEllipsis" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawTextEllipsis(true).unwrap());
    defer item.deinit();
...
test "ColorBar DragSource" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDragSource(true).unwrap());
    defer item.deinit();
...
test "ColorBar DrawTextClip" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawTextClip(true).unwrap());
    defer item.deinit();
...
test "ColorBar Floating" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setFloating(.Yes).unwrap());
    defer item.deinit();
...
test "ColorBar NormalizerGroup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setNormalizerGroup("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar RasterSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setRasterSize(9, 10).unwrap());
    defer item.deinit();
...
test "ColorBar TipFgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setTipFgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "ColorBar FontFace" {
    ...
    var item = try (iup.ColorBar.init().setFontFace("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar DrawColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "ColorBar DrawTextOrientation" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawTextOrientation(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar DrawBgColor" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawBgColor(.{ .r = 9, .g = 10, .b = 11 }).unwrap());
    defer item.deinit();
...
test "ColorBar Name" {
    ...
    var item = try (iup.ColorBar.init().setName("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar BackingStore" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setBackingStore(true).unwrap());
    defer item.deinit();
...
test "ColorBar YAutoHide" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setYAutoHide(true).unwrap());
    defer item.deinit();
...
test "ColorBar DrawStyle" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDrawStyle(.Fill).unwrap());
    defer item.deinit();
...
test "ColorBar Active" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setActive(true).unwrap());
    defer item.deinit();
...
test "ColorBar TipVisible" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setTipVisible(true).unwrap());
    defer item.deinit();
...
test "ColorBar YMax" {
    ...
    var item = try (iup.ColorBar.init().setYMax(42).unwrap());
    defer item.deinit();
...
test "ColorBar ExpandWeight" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setExpandWeight(3.14).unwrap());
    defer item.deinit();
...
test "ColorBar MinSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setMinSize(9, 10).unwrap());
    defer item.deinit();
...
test "ColorBar Flat" {
    ...
    var item = try (iup.ColorBar.init().setFlat(true).unwrap());
    defer item.deinit();
...
test "ColorBar NTheme" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setNTheme("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar DragTypes" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setDragTypes("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar WheelDropFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setWheelDropFocus(true).unwrap());
    defer item.deinit();
...
test "ColorBar FontStyle" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ColorBar.init().setFontStyle("Hello").unwrap());
    defer item.deinit();
...
test "ColorBar Touch" {
    ...
    var item = try (iup.ColorBar.init().setTouch(true).unwrap());
    defer item.deinit();
...
test "ColorBar Font" {
    ...
    var item = try (iup.ColorBar.init().setFont("Hello").unwrap());
    defer item.deinit();
