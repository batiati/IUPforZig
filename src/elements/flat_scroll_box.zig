...
pub const FlatScrollBox = opaque {
    ...
    pub const Initializer = struct {
        ...
        /// LAYOUTUPDATE_CB: Action generated when the layout is updated after a scroll operation.
        /// (since 3.24)
        pub fn setLayoutUpDateCallback(self: Initializer, callback: ?*const OnLayoutUpDateFn) Initializer {
    ...
    /// LAYOUTUPDATE_CB: Action generated when the layout is updated after a scroll operation.
    /// (since 3.24)
    pub fn setLayoutUpDateCallback(self: *Self, callback: ?*const OnLayoutUpDateFn) void {
        const Handler = CallbackHandler(Self, OnLayoutUpDateFn, "LAYOUTUPDATE_CB");
        Handler.setCallback(self, callback);
    }
...
test "FlatScrollBox TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatScrollBox.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "FlatScrollBox TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.FlatScrollBox.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
