...
pub const HBox = opaque {
    pub const CLASS_NAME = "hbox";
    ...
    pub const Initializer = struct {
        ...
        /// Default: "NO".
        /// This has the same effect as setting EXPAND=VERTICAL on each child.
        /// (since 3.0)
        ...
        pub fn setAlignment(self: *Initializer, arg: ?Alignment) Initializer {
            if (self.last_error) |_| return self.*;
            if (arg) |value| switch (value) {
                .ABottom => interop.setStrAttribute(self.ref, "ALIGNMENT", .{}, "ABOTTOM"),
                .ACenter => interop.setStrAttribute(self.ref, "ALIGNMENT", .{}, "ACENTER"),
                .ATop => interop.setStrAttribute(self.ref, "ALIGNMENT", .{}, "ATOP"),
            } else {
    ...
    pub fn getAlignment(self: *Self) ?Alignment {
        ...
        if (std.ascii.eqlIgnoreCase("ABOTTOM", ret)) return .ABottom;
    ...
    pub fn setAlignment(self: *Self, arg: ?Alignment) void {
        if (arg) |value| switch (value) {
            .ABottom => interop.setStrAttribute(self, "ALIGNMENT", .{}, "ABOTTOM"),
            .ACenter => interop.setStrAttribute(self, "ALIGNMENT", .{}, "ACENTER"),
            .ATop => interop.setStrAttribute(self, "ALIGNMENT", .{}, "ATOP"),
        } else {
...
test "HBox UserSize" {
    ...
    var item = try (iup.HBox.init().setUserSize(9, 10).unwrap());
    defer item.deinit();
...
test "HBox FontStyle" {
    ...
    var item = try (iup.HBox.init().setFontStyle("Hello").unwrap());
    defer item.deinit();
...
test "HBox FontSize" {
    ...
    var item = try (iup.HBox.init().setFontSize(42).unwrap());
    defer item.deinit();
...
test "HBox ExpandWeight" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setExpandWeight(3.14).unwrap());
    defer item.deinit();
...
test "HBox Margin" {
    ...
    var item = try (iup.HBox.init().setMargin(9, 10).unwrap());
    defer item.deinit();
...
test "HBox MaxSize" {
    ...
    var item = try (iup.HBox.init().setMaxSize(9, 10).unwrap());
    defer item.deinit();
...
test "HBox NormalizeSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setNormalizeSize(.Horizontal).unwrap());
    defer item.deinit();
...
test "HBox NGap" {
    ...
    var item = try (iup.HBox.init().setNGap(42).unwrap());
    defer item.deinit();
...
test "HBox Active" {
    ...
    var item = try (iup.HBox.init().setActive(true).unwrap());
    defer item.deinit();
...
test "HBox NTheme" {
    ...
    var item = try (iup.HBox.init().setNTheme("Hello").unwrap());
    defer item.deinit();
...
test "HBox ExpandChildren" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setExpandChildren(true).unwrap());
    defer item.deinit();
...
test "HBox Gap" {
    ...
    var item = try (iup.HBox.init().setGap(42).unwrap());
    defer item.deinit();
...
test "HBox CMargin" {
    ...
    var item = try (iup.HBox.init().setCMargin(9, 10).unwrap());
    defer item.deinit();
...
test "HBox Visible" {
    ...
    var item = try (iup.HBox.init().setVisible(true).unwrap());
    defer item.deinit();
...
test "HBox Homogeneous" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setHomogeneous(true).unwrap());
    defer item.deinit();
...
test "HBox FontFace" {
    ...
    var item = try (iup.HBox.init().setFontFace("Hello").unwrap());
    defer item.deinit();
...
test "HBox PropagateFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setPropagateFocus(true).unwrap());
    defer item.deinit();
...
test "HBox HandleName" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setHandleName("Hello").unwrap());
    defer item.deinit();
...
test "HBox Size" {
    ...
    var item = try (iup.HBox.init().setSize(42).unwrap());
    defer item.deinit();
...
test "HBox NormalizerGroup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setNormalizerGroup("Hello").unwrap());
    defer item.deinit();
...
test "HBox CGap" {
    ...
    var item = try (iup.HBox.init().setCGap(42).unwrap());
    defer item.deinit();
...
test "HBox Font" {
    ...
    var item = try (iup.HBox.init().setFont("Hello").unwrap());
    defer item.deinit();
...
test "HBox Name" {
    ...
    var item = try (iup.HBox.init().setName("Hello").unwrap());
    defer item.deinit();
...
test "HBox NMargin" {
    ...
    var item = try (iup.HBox.init().setNMargin(9, 10).unwrap());
    defer item.deinit();
...
test "HBox NcMargin" {
    ...
    var item = try (iup.HBox.init().setNcMargin(9, 10).unwrap());
    defer item.deinit();
...
test "HBox Expand" {
    ...
    var item = try (iup.HBox.init().setExpand(.Yes).unwrap());
    defer item.deinit();
...
test "HBox CanFocus" {
    ...
    var item = try (iup.HBox.init().setCanFocus(true).unwrap());
    defer item.deinit();
...
test "HBox RasterSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.HBox.init().setRasterSize(9, 10).unwrap());
    defer item.deinit();
...
test "HBox Floating" {
    ...
    var item = try (iup.HBox.init().setFloating(.Yes).unwrap());
    defer item.deinit();
...
test "HBox Theme" {
    ...
    var item = try (iup.HBox.init().setTheme("Hello").unwrap());
    defer item.deinit();
...
test "HBox NcGap" {
    ...
    var item = try (iup.HBox.init().setNcGap(42).unwrap());
    defer item.deinit();
...
test "HBox MinSize" {
    ...
    var item = try (iup.HBox.init().setMinSize(9, 10).unwrap());
    defer item.deinit();
...
test "HBox Position" {
    ...
    var item = try (iup.HBox.init().setPosition(9, 10).unwrap());
    defer item.deinit();
...
test "HBox Alignment" {
    ...
    var item = try (iup.HBox.init().setAlignment(.ABottom).unwrap());
    defer item.deinit();

    var ret = item.getAlignment();

    try std.testing.expect(ret != null and ret.? == .ABottom);
}
