...
pub const Fill = opaque {
    pub const CLASS_NAME = "fill";
    const Self = @This();
    ...
    /// 
    /// EXPAND (non inheritable)(read-only): If User size is not defined, then when
    /// inside a IupHbox/IupGridBox EXPAND is HORIZONTAL, when inside a IupVbox
    /// EXPAND is VERTICAL.
    ...
    pub const Initializer = struct {
        ...
        /// 
        /// SIZE / RASTERSIZE (non inheritable): Defines the width, if inside a
        /// IupHbox, or the height, if it is inside a IupVbox.
        /// The standard format "wxh" can also be used, but width will be ignored if
        /// inside a IupVbox and height will be ignored if inside a IupHbox (since 3.3).
...
test "Fill UserSize" {
    ...
    var item = try (iup.Fill.init().setUserSize(9, 10).unwrap());
    defer item.deinit();
...
test "Fill FontStyle" {
    ...
    var item = try (iup.Fill.init().setFontStyle("Hello").unwrap());
    defer item.deinit();
...
test "Fill FontSize" {
    ...
    var item = try (iup.Fill.init().setFontSize(42).unwrap());
    defer item.deinit();
...
test "Fill ExpandWeight" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Fill.init().setExpandWeight(3.14).unwrap());
    defer item.deinit();
...
test "Fill MaxSize" {
    ...
    var item = try (iup.Fill.init().setMaxSize(9, 10).unwrap());
    defer item.deinit();
...
test "Fill Active" {
    ...
    var item = try (iup.Fill.init().setActive(true).unwrap());
    defer item.deinit();
...
test "Fill NTheme" {
    ...
    var item = try (iup.Fill.init().setNTheme("Hello").unwrap());
    defer item.deinit();
...
test "Fill Visible" {
    ...
    var item = try (iup.Fill.init().setVisible(true).unwrap());
    defer item.deinit();
...
test "Fill FontFace" {
    ...
    var item = try (iup.Fill.init().setFontFace("Hello").unwrap());
    defer item.deinit();
...
test "Fill PropagateFocus" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Fill.init().setPropagateFocus(true).unwrap());
    defer item.deinit();
...
test "Fill HandleName" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Fill.init().setHandleName("Hello").unwrap());
    defer item.deinit();
...
test "Fill Size" {
    ...
    var item = try (iup.Fill.init().setSize(9, 10).unwrap());
    defer item.deinit();
...
test "Fill NormalizerGroup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Fill.init().setNormalizerGroup("Hello").unwrap());
    defer item.deinit();
...
test "Fill Font" {
    ...
    var item = try (iup.Fill.init().setFont("Hello").unwrap());
    defer item.deinit();
...
test "Fill Name" {
    ...
    var item = try (iup.Fill.init().setName("Hello").unwrap());
    defer item.deinit();
...
test "Fill Expand" {
    ...
    var item = try (iup.Fill.init().setExpand(.Yes).unwrap());
    defer item.deinit();
...
test "Fill CanFocus" {
    ...
    var item = try (iup.Fill.init().setCanFocus(true).unwrap());
    defer item.deinit();
...
test "Fill RasterSize" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Fill.init().setRasterSize(9, 10).unwrap());
    defer item.deinit();
...
test "Fill Floating" {
    ...
    var item = try (iup.Fill.init().setFloating(.Yes).unwrap());
    defer item.deinit();
...
test "Fill Theme" {
    ...
    var item = try (iup.Fill.init().setTheme("Hello").unwrap());
    defer item.deinit();
...
test "Fill MinSize" {
    ...
    var item = try (iup.Fill.init().setMinSize(9, 10).unwrap());
    defer item.deinit();
...
test "Fill Position" {
    ...
    var item = try (iup.Fill.init().setPosition(9, 10).unwrap());
    defer item.deinit();
