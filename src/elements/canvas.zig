...
pub const Canvas = opaque {
    ...
    pub inline fn drawPolygon(self: *Self, points: []const i32) void {
        interop.drawPolygon(self, points.ptr, @as(c_int, @intCast(points.len)));
    }
...
test "Canvas TipIcon" {
    ...
    var item = try (iup.Canvas.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "Canvas TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.Canvas.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
