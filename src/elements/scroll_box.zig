...
test "ScrollBox TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ScrollBox.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "ScrollBox TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.ScrollBox.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
