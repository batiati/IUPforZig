...
test "BackgroundBox TipIcon" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.BackgroundBox.init().setTipIcon("Hello").unwrap());
    defer item.deinit();
...
test "BackgroundBox TipMarkup" {
    try iup.MainLoop.open();
    defer iup.MainLoop.close();

    var item = try (iup.BackgroundBox.init().setTipMarkup("Hello").unwrap());
    defer item.deinit();
